...
@property (nonatomic, strong)UILabel *noteSubTitle;
@property (nonatomic, strong)UIView *noteView;//蒙版
...
-(void)buidView{
    ...
    //_noteSubTitle
    [_noteView addSubview:self.noteSubTitle];
    self.clipsToBounds = YES;
    
}
-(void)layoutSubviews{
    _scrollView.frame = CGRectMake(0, 0, self.frame.size.width, self.frame.size.height);
    _noteView.frame =CGRectMake(0, 0,self.bounds.size.width,self.bounds.size.height);
    _scrollView.contentSize =CGSizeMake(self.frame.size.width*3, self.frame.size.height);
    
    float pageControlWidth = ([self.items count]) * 10.0f + 60.f;
    float pagecontrolHeight = 20.0f;
    _pageControl.frame = CGRectMake((self.frame.size.width - pageControlWidth)/2, self.frame.size.height - pagecontrolHeight - 10, pageControlWidth, pagecontrolHeight);
    _noteTitle.frame = CGRectMake(kLeadingMargin, 80* autoLayoutY, self.frame.size.width - 2* kLeadingMargin, 20* autoLayoutY);
    _noteSubTitle.frame = CGRectMake(kLeadingMargin, 100* autoLayoutY + 10, self.frame.size.width - 2* kLeadingMargin, 15* autoLayoutY);
...
-(UIView *)noteView{
    if (!_noteView) {
        ...
        _noteView.userInteractionEnabled = NO;//模版不接收点击事件
        _noteView.alpha = 0.4;
    }
    return _noteView;
}
...
-(UILabel *)noteTitle{
    if (!_noteTitle) {
        ...
        _noteTitle.textColor = kWhiteColor;
        [_noteTitle setFont:kFont(16* autoLayoutY)];
        _noteTitle.textAlignment = NSTextAlignmentCenter;
    }
    return _noteTitle;
}
-(UILabel *)noteSubTitle{
    
    if (!_noteSubTitle) {
        _noteSubTitle = [[UILabel alloc] initWithFrame:CGRectZero];
        [_noteSubTitle setBackgroundColor:[UIColor clearColor]];
        _noteSubTitle.textColor = kWhiteColor;
        [_noteSubTitle setFont:kFont(12* autoLayoutY)];
        _noteSubTitle.textAlignment = NSTextAlignmentCenter;
    }
    return _noteSubTitle;
}
...
-(void)reloadData{
    ...
    if (!_titleForJKScrollFocusItem) {
        ...
        _noteSubTitle.text = _titleForJKScrollFocusItem([self.items firstObject],self.noteSubTitle);
    }
...
- (void)moveToPage:(NSInteger)page{
    if (_titleForJKScrollFocusItem  && [self.items count]>page) {
        ...
        _noteSubTitle.text = _titleForJKScrollFocusItem([self.items objectAtIndex:page],_noteSubTitle);
    }
