...
- (void)loadWithCompletionBlock:(void(^)(UIImage *image, NSURL *url, NSError *error))handler {
    ...
    dispatch_async(self.cachingQueue, ^{
        UIImage *cacheImage = [[YYWebImageManager sharedManager].cache getImageForKey:weakSelf.url.absoluteString];
        if (cacheImage) {
            ...
            [[YYWebImageManager sharedManager] requestImageWithURL:weakSelf.url options:YYWebImageOptionProgressiveBlur | YYWebImageOptionSetImageWithFadeAnimation progress:^(NSInteger receivedSize, NSInteger expectedSize) {
                
            } transform:^UIImage * _Nullable(UIImage * _Nonnull image, NSURL * _Nonnull url) {
                return image;
            } completion:^(UIImage * _Nullable image, NSURL * _Nonnull url, YYWebImageFromType from, YYWebImageStage stage, NSError * _Nullable error) {
                [weakSelf didFinishDownload:image forURL:url error:error];
                dispatch_async(dispatch_get_main_queue(), ^{
                    if(handler) {
...
- (void)cachingImage:(UIImage *)image url:(NSURL *)url {
    dispatch_async(self.cachingQueue, ^{
        if (image) {
            NSData *data = UIImagePNGRepresentation(image);
            [[YYWebImageManager sharedManager].cache setImage:image imageData:data forKey:url.absoluteString withType:YYImageCacheTypeAll];
        }
    });
}

- (UIImage *)didFinishDownload:(UIImage *)image forURL:(NSURL *)url error:(NSError *)error {
    if (image) {
        [self cachingImage:image url:url];
    }
    if([url isEqual:self.url]) {
        if(error) {
